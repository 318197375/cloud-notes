...
instance.interceptors.request.use((config) => {
  ...
  const isLoginOrRegister = config.url.includes("/login") || config.url.includes("/register")
    || config.url.includes("/userInfoByName") || config.url.includes("/sendRegisterCaptcha") || config.url.includes("/sendRePwdCaptcha")
    || config.url.includes("/verifyUserName")|| config.url.includes("/updatePassword")
